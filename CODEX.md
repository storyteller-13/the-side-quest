...
Tests live in `tests/` (Vitest): config shape and content, scores API (CORS, methods, no-KV behaviour, `sanitizeName`), game helpers (`zoneDisplayName`, `mixHex`), and index.html structure. A **pre-commit hook** (husky) runs tests and coverage on each commit; coverage thresholds in `vitest.config.js` must pass. **CI** (`.github/workflows/test.yml`) runs the same on push/PR to main. If the hook doesn’t run, run `make install-hooks` once. Run `make pre-commit` for the same check without committing.
